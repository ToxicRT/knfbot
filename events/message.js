...
module.exports = async (client, message) => {
    ...
    sql.get(`SELECT * FROM scores WHERE guildId ="${message.guild.id}"`).then(row => {
        if (!row) return;
            ...
            if (!message.guild.member(client.user).hasPermission('VIEW_CHANNEL')) return;
             ...
             if (row.modonlycommands === "enabled") {
                ...
                const command = args.shift().toLowerCase();
                    console.log(`[KnightNoFansubBOT] [${message.guild.name}] [${message.author.username}] ${row.prefix}${command} ${args}`);
                    try {
            ...
            const args = message.content.slice(prefix.length).trim().split(/ +/g);
            const command = args.shift().toLowerCase();
                console.log(`[KNFB] [${message.guild.name}] [${message.author.username}] ${row.prefix}${command} ${args}`);
                try {
                    let commandFile = require(`../commands/${command}.js`);
                    commandFile.run(client, message, args);
                } catch (err) {
